...
class SendConfig(BaseModel):
    ...
    interval: WaitRange
    #: **直近これだけの日数に媒体からスカウトを送っている相手には送らない。**
    #:
    #: 見るのは **媒体側の履歴** であってこちらの送信記録ではない。自動化を
    #: 始める前に人手で送った分はこちらの記録に無く、媒体側にしか無いからで
    #: ある (実測51回目)。
    #:
    #: 判定不能のときは **送らない側へ倒す**。取り消せないのは送るほうで、
    #: 送らないのは次回やり直せる。倒したことは必ず報告に出す (原則2)。
    #: 0 にするとこの判定は「今日送った相手だけを外す」になる。**無効化する
    #: 手段は用意していない** -- 閾値を寛容な値へ書き換えて実質無効にする形は
    #: 7.6 が禁じている (意図なのか打鍵ミスなのか読めなくなる)。
