...
#: 目印を書き込む先のタグ。**``textarea`` だけである。**
#:
#: 最初は ``input`` も対象にしていた。「書き込みは失敗しても何も起きない」から
#: 種別で絞らなくてよい、という理屈だった。**実測13回目でそれが誤りだと分かった。**
#:
#: この画面の ``input.c-text-field`` は **求人を探すサジェスト欄** である。
#: そこへ目印の本文 (複数行のダミー文) を書き込んだ結果:
#:
#: * ``JobOfferSearchOnScoutMessageForm`` が **54回** 走った
#: * 該当する求人は当然1件も無く、``div.c-suggest__list`` は空のまま消えた
#: * 求人が選ばれないので、送信は ``ul.js-validation-error-message`` で弾かれた
#:
#: **書き込みは「失敗しても何も起きない」ではなかった。** 検索欄に無関係な文字列を
#: 入れる操作は、成功したうえでフォームを壊す。
#:
#: ``textarea`` に限る根拠は用途である -- 長文を受ける欄はメッセージ本文であり、
#: 目印を載せたいのもそこ (送信リクエストの本文に現れるのは本文である)。
#: 短い値を受ける ``input`` は検索・件名・チェックボックスのいずれかで、
#: どれも目印を書く先ではない。
